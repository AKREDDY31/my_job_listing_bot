...
import os
import time
import telegram # NEW: Using python-telegram-bot v20+
from telegram import Bot # NEW: Explicit import
from telegram.constants import ParseMode # NEW: For MarkdownV2
from telegram.error import TelegramError, RetryAfter # NEW: Updated error types
import asyncio # NEW: Required for async operations
...
import re
import requests
import httpx # NEW: Often needed by PTB v20+ for async requests
...
ALL_JOB_FEEDS = [
    # Add your chosen RSS Feeds Here (Indeed, Govt Blogs, etc.)
...
def load_all_seen_jobs():
    # (Same function as before - handles loading seen job links from JSON)
...
def save_all_seen_jobs(seen_links):
    # (Same function as before - saves seen job links to JSON)
    try:
...
async def send_job_to_channel_async(title, link, summary):
    ...
    # --- Initialize Bot within the async function ---
    # Configure timeouts using httpx
    timeout = httpx.Timeout(15.0, read=30.0) # connect, read
    bot = Bot(token=TELEGRAM_TOKEN, request=telegram.request.HTTPXRequest(http_version="1.1", timeout=timeout)) # Use HTTPXRequest
    ...
    # Escape function for MarkdownV2 (PTB v20 doesn't have a built-in one easily accessible)
    def escape_markdown_v2(text):
        escape_chars = r'_*[]()~`>#+-=|{}.!'
        return re.sub(f'([{re.escape(escape_chars)}])', r'\\\1', text)

    safe_title = escape_markdown_v2(title)
    safe_link = escape_markdown_v2(link) # Escape link too, just in case
    ...
    message = (
        ...
        # Ensure link itself isn't escaped within the markdown link syntax
        f"*Link:* [Apply Here]({link.replace(')', '%29').replace('(', '%28')})" # URL Encode problematic chars
    )

    try:
        # NEW: Use await for async call
        ...
        await asyncio.sleep(wait_time + 1) # NEW: use asyncio.sleep
        print(f"  > Will retry posting '{title}' later.", flush=True)
        return False # Indicate failure for re-queuing
    except TelegramError as e: # Catch broader Telegram errors
        print(f"  > FAILED sending '{title}'. Telegram Error: {type(e).__name__} - {e}", flush=True)
        # Consider specific error handling (e.g., BadRequest for Markdown issues)
        # Fallback to plain text could be added here if needed
        return False # Indicate failure for re-queuing
    except Exception as e:
...
def fetch_unstop_jobs():
    # (Same function as before - fetches from Unstop API using requests)
...
def check_sources_for_new_jobs():
    # (Same function as before - checks RSS feeds and Unstop, adds new to queue)
...
def post_one_job_from_queue():
    ...
    if job_to_post:
        try:
            # NEW: Run the async function using asyncio.run()
            success = asyncio.run(send_job_to_channel_async(
                job_to_post['title'], job_to_post['link'], job_to_post['summary']
            ))

            # If sending failed (e.g., flood control, other errors), re-queue at the front
            if not success:
                print(f"  > Posting failed for '{job_to_post['title']}'. Re-queuing at front.", flush=True)
                with queue_lock:
                    pending_jobs_queue.appendleft(job_to_post)
        except RuntimeError as e:
            # Handle cases where asyncio.run might fail if another loop is running (less likely with BackgroundScheduler)
...
def run_flask_app():
    # (Same function as before - runs Waitress or Flask dev server)
...
if __name__ == "__main__":
    print("--- Initializing Job Agent (Async Telegram Version) ---", flush=True)
